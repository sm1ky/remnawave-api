...
class CreateUserBodyDto(BaseModel):
    """Request body for creating a user (Remnawave API v3.0.0)."""
    username: Annotated[
        str,
...
class UpdateUserBodyDto(BaseModel):
    """Request body for updating a user (identified by numeric id in v3.0.0)."""
    id: Optional[int] = Field(None, description="Numeric id of the user")
...
class ExtendUserBodyDto(BaseModel):
    """Request body for extending a user's expiration date (v3.0.0)."""
    days: int = Field(..., ge=1, description="Number of days to extend by")


class RevokeUserSubscriptionBodyDto(BaseModel):
    """Request body for revoking a user subscription."""
...
class ResolveUserBodyDto(BaseModel):
    """Request body for resolving a user by any identifier (id, shortUuid, username)."""
...
class UserResponseDto(BaseModel):
    """User response DTO (Remnawave API v3.0.0 — identified by numeric ``id``, ``uuid`` removed)."""
    id: int
    short_uuid: str = Field(alias="shortUuid")
    username: str
...
class GetUsersResponseDto(UsersResponseDto):
...
class GetUsersTagsResponseDto(TagsResponseDto):
    """Response for get all user tags"""
    pass
